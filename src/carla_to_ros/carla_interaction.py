...
class CarlaGame(object):
    ...
    def _on_loop(self):
        ...
        # Getting the data from UE4
        measurements, sensor_data = self.client.read_data()

        self._main_image = sensor_data['CameraRGB']
        # Note(alexmillane): Taken out to (hopefully) save render time.
        #self._mini_view_image1 = sensor_data['CameraDepth']
        #self._mini_view_image2 = sensor_data['CameraSemSeg']

        # Calling the registered sensor call back
        if self._data_callback is not None:
            self._data_callback(measurements, sensor_data)
